...
export default class TeamList extends React.PureComponent<Props, State> {
    ...
    getPaginationProps = () => {
        const {page, term, filters, page_size} = this.state;
    ...
    loadPage = async (page = 0, term = '', filters = {}) => {
        ...
        // const res: any = await this.props.actions.getData(page, PAGE_SIZE);

        // let ids = this.props.myTeams.map(team => team.id)
        // let teams = Object.values(res.data).(team => team.delete_at == 0 && (!ids.includes(team.id)) )
    ...
    getRows = () => {
        ...
        return teamsToDisplay.map((team) => {
            return {
                ...
                onClick: () => this.handleClickRow(team),
            };
        });
    }
